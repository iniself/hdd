...
class Container(object):
    ...
    def get_alias(self, abstract):
        return (
            self.get_alias(self.aliases[abstract])
            if (not is_list(abstract) and abstract in self.aliases)
            else abstract
        )

    def is_shared(self, abstract, is_share_in_alias):
        return (
            (abstract in self._instances)
            or (is_share_in_alias)
            or (abstract in self._binds and self._binds[abstract]["shared"])
        )
