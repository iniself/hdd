...
class EventHub:
    ...
    def watch(self, event, handler, always=False):
        if not event in self._event:
            print(
                f"[EventHub] watch，attempt to watch an invalid event：{event}. List of valid events: {self._event}"
            )
            return
    ...
    def fire(self, event, data=None):
        if not event in self._event:
            print(
                f"[EventHub] fire，attempt to trigger an invalid event：{event}. List of valid events: {self._event}"
            )
            return
        ...
        for listener in self._listeners:
            ...
            try:
                ...
                print(
                    f"[EventHub] caught err when exec handler, err:, {e}, event:, {event}, handler:, {listener.handler}"
                )
        ...
        self._listeners = [
            ...
            for listener in self._listeners
            if not (
                listener["limit_count"] > 0
                and listener["limit_count"] <= listener["trigger_count"]
            )
        ]

    def unwatch(self, event, handler):
        self._listeners = [
            listener for listener in self._listeners if not (listener["event"] == event)
        ]
