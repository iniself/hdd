...
import hdd.snippet as snippet
from hdd.ioc import Container
...
class Core(Container):
    def __init__(
        self, package_absolute_path=None, project_absolute_path=None, aliases=dict
    ):
        super().__init__()
    ...
    def _set_base_path(self, package_absolute_path, project_absolute_path):
        self._package_absolute_path = (
            package_absolute_path
        ).is_absolute() and self._check_absolute_path_else_return_relative_path(
            package_absolute_path
        )
        self._project_absolute_path = (
    ...
    def _bind_paths_in_core(self, path=""):
        ...
        self.instance(
            "package.config.absolute.path",
            self._package_absolute_path.joinpath("config"),
        )
        self.instance(
            "package.bootstrap.absolute.path",
            self._package_absolute_path.joinpath("bootstrap"),
        )
        self.instance(
            "package.provider.absolute.path",
            self._package_absolute_path.joinpath("provider"),
        )

        self.instance("project.base.absolute.path", self._project_absolute_path)
        self.instance(
            "project.config.absolute.path",
            self._project_absolute_path.joinpath("config"),
        )
        self.instance(
            "project.bootstrap.absolute.path",
            self._project_absolute_path.joinpath("bootstrap"),
        )
        self.instance(
            "project.provider.absolute.path",
            self._project_absolute_path.joinpath("provider"),
        )
        self.instance(
            "project.migrate.absolute.path",
            self._project_absolute_path.joinpath("migrate"),
        )
    ...
    def _register_core_service_providers(self):
        ...
        self.instance(
            "event", snippet.dyn_from_path_import("hdd.service.event.EventHub")
        )
    ...
    def bootstrap(self, bootstrappers):
        ...
        if connector and connector.lower() == "csv":
            self.singleton("hdd.interface.abs.DBManagerAbs", "hdd.interface.dber.CSV")
        elif connector and connector.lower() == "xls":
            self.singleton("hdd.interface.abs.DBManagerAbs", "hdd.interface.dber.XLS")
        else:
            self.singleton(
                "hdd.interface.abs.DBManagerAbs", "hdd.interface.dber.DBManager"
            )
